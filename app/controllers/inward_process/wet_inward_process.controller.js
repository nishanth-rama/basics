...
async function get_inwarded_po_asn_sku_list(inward_filter) {
  const getInwardDetails = await inwardProcess
    .find(
      inward_filter,
      {
...
async function get_complete_po_asn_sku_list(po_filter, asn_no) {
  let sku_list = [];
  if (!asn_no) {
    sku_list = await purchase_order_table.aggregate([{
      $match: po_filter
    }, { $unwind: "$item" },
    {
      $lookup: {
        ...
        foreignField: "po_no",
        let: { po_item_no: "$item.item_no" },
        pipeline: [{
          $match: {
            $expr: { $eq: ["$item_no", "$$po_item_no"] }
          }
        }, { $limit: 1 }],
        as: "inward_details"
      }
      ...
      $project: {
        ...
        total_inwarded_qty: "$inward_details.total_inwarded_qty",
        total_net_qty: "$inward_details.total_net_qty",
        total_pending_qty: "$inward_details.total_pending_qty",
        total_crates: "$inward_details.total_crates",
        uom: "$item.uom"
      }
    }]);
  } else {
    sku_list = await asn_table.aggregate([{
      $match: { po_number: po_filter.po_number }
    }, { $unwind: "$item" },
    { $match: { "item.inbound_delivery_number": asn_no } },
    {
      $group: {
        _id: "$item.inbound_delivery_item_no",
        inbound_delivery_qty: {
          $first: "$item.inbound_delivery_qty"
        },
        po_number: { $first: "$po_number" },
        material: { $first: "$item.material" },
        item_name: { $first: "$item.material_description" },
        item_no: { $first: "$item.inbound_delivery_item_no" },
        ordered_qty: { $first: "$item.po_qty" },
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [{
          $match: {
            $expr: { $eq: ["$asn_item_no", "$$asn_item_no"] }
          }
        }, { $limit: 1 }],
        as: "inward_details",
      }
    },
    {
      $lookup: {
        ...
        pipeline: [{
          ...
          $match: {
            $expr: { $eq: ["$item.material_no", "$$material"] }
          }
        }, { $limit: 1 }],
        as: "po_details"
      }
...
exports.real_time_vendor_inward_details_v2 = async (req, res) => {
  console.log("real_time_vendor_inward_details_v2");
  const { company_code, plant_id, po_no, asn_no, mode, manual_inward_status } = req.query;
  try {
    if (!(company_code && plant_id && po_no && mode))
      return res.status(400).send({
        status_code: 400,
    ...
    let po_details = await purchase_order_table.findOne(po_filter);
    ...
    var response_data = [];
    //console.log("manual_inward_status",manual_inward_status,typeof manual_inward_status)
    if (manual_inward_status == "true") {
      response_data = await get_complete_po_asn_sku_list(po_filter, asn_no);
    }
    else if (manual_inward_status == "false") {
      response_data = await get_inwarded_po_asn_sku_list(inward_filter);
    }
    ...
    response_data.forEach((element) => {
      ...
      x.total_pending_qty = element.total_pending_qty
        ? element.total_pending_qty
        : element.ordered_qty == element.po_qty ? element.po_qty : element.ordered_qty;
      x.total_crates = element.total_crates ? element.total_crates : 0;
      if (!(x.total_inwarded_qty)) {
        x.status = "Pending";
      } else if (x.total_pending_qty) {
        x.status = "Partially Completed";
      } else {
        x.status = "Completed";
      }
...
exports.real_time_vendor_inward_details_v3 = async (req, res) => {
  console.log("real_time_vendor_inward_details_v3");
  const { company_code, plant_id, po_no, asn_no, mode } = req.query;
  try {
    if (!(company_code && plant_id && po_no && mode))
      return res.status(400).send({
    ...
    let manual_inward_status = await db.company_plant_config.findOne({
      company_code: company_code,
      plant_id: plant_id
    }, { _id: 0, manual_inward: 1 });
    ...
    var response_data = [];
    //console.log("manual_inward_status",manual_inward_status,typeof manual_inward_status)
    if (manual_inward_status.manual_inward) {
      response_data = await get_complete_po_asn_sku_list(po_filter, asn_no);
    }
    else {
      response_data = await get_inwarded_po_asn_sku_list(inward_filter);
...
exports.save_manual_vendor_inward_process_v3 = async (req, res) => {
  console.log("calling save manual inward process api");
  ...
  try {
    ...
    for (let i = 0; i < inwardDetailsArr.length; i++) {
      ...
      let actual_qty = inwardDetailsArr[i].actual_qty;
      let carrier_count = inwardDetailsArr[i].carrier_count;
      //let asn_item_no = inwardDetailsArr[i].asn_item_no;
      ...
      if (checkExistingEntry == null) {
        ...
        if (getPoDetails.length == 0) {
          ...
          let newDataEntry = {
            ...
            supplier_no: getPoDetails[0].vendor_no,
            po_type: getPoDetails[0].po_document_type,
            purchase_group:getPoDetails[0].purchase_group,
            document_date:getPoDetails[0].document_date,
            po_no: po_no,
            item_code: item_code,
            item_no: item_no,
            item_name: getPoDetails[0].item["material_description"],
            ordered_qty: order_qty,
            po_qty:getPoDetails[0].item["quantity"],
            uom: getPoDetails[0].item["uom"],
            unit_price: getPoDetails[0].item["net_price"],
            actual_qty: actual_qty,
            invoice_no: "",
          ...
          if (asn_no){
            newDataEntry.inbound_delivery_number = asn_no;
            newDataEntry.asn_item_no = item_no;
          }
...
async function get_inwarded_po_asn_list(filter) {
  const poNumbers = (
    await inwardProcess.aggregate([
      { $match: filter },
      { $sort: { updatedAt: -1 } },
...
async function get_po_asn_details(filter) {

  let po_asn_data = await purchase_order_table.aggregate([
    { $match: filter },
    { $sort: { updatedAt: -1 } },
    //{ $project: { _id: 0, po_number: 1 } },
    {
      $lookup: {
    ...
    },
    {
      $project: {
        _id: 0,
        po_number: "$po_number",
        inbound_delivery_number: "$asn_details.item.inbound_delivery_number"
      }
    },
    {
      $unwind: {
        path: "$inbound_delivery_number",
        preserveNullAndEmptyArrays: true
      }
    },
    {
    ...
    },
    {
      $group: {
        ...
        po_number: { $first: "$po_number" },
        inbound_delivery_number: { $first: "$inbound_delivery_number" }
      }
    },
    {
      $project: {
  ...
  //console.log("po_asn_data",po_asn_data);
  return po_asn_data;

}

async function get_grn_manual_inward_status_details(element) {
  //console.log("element", element);
  let inbound_delivery_number = element.inbound_delivery_number;
  let po_number = element.po_number;
  let manual_inward_flag = 1;
  let grn_status_flag = 0;
  let sto_flag = 1;

  if (inbound_delivery_number) {
    let get_asn_details = await asn_table.aggregate([
      { $unwind: "$item" }, {
        $match: {
          "item.inbound_delivery_number": inbound_delivery_number
        }
      }, {
        $unwind: "$item"
      }, {
        $group: {
          _id: "$item.inbound_delivery_item_no",
          material: { $first: "$item.material" },
          inbound_delivery_item_no: { $first: "$item.inbound_delivery_item_no" }
        }
      }]);
    //console.log("get_asn_details", get_asn_details, inbound_delivery_number);

    let temp_asn_item_array = [];
    get_asn_details.forEach(asn_element => {
      temp_asn_item_array.push(asn_element.inbound_delivery_item_no);
    })

    //console.log("temp_asn_item_array", temp_asn_item_array);
    let inward_asn_item_details = await inwardProcess.aggregate([
      {
        $match: {
          asn_item_no: { $in: temp_asn_item_array },
          inbound_delivery_number: inbound_delivery_number
        }
      },
      {
        $group: {
          _id: "$inbound_delivery_number",
          total_item: { $sum: 1 },
          total_grn_post_qty: { $sum: "$total_grn_post_qty" },
          total_net_qty: { $sum: "$total_net_qty" },
          total_pending_qty: { $sum: "$total_pending_qty" }
        }
      }
    ]);

    //console.log("inward_asn_item_details", inward_asn_item_details);

    let asn_details_length = get_asn_details.length;
    let inward_asn_item_details_length = inward_asn_item_details.length;

    if (inward_asn_item_details_length)
    {
      grn_status_flag = inward_asn_item_details[0].total_grn_post_qty != inward_asn_item_details[0].total_net_qty ? 1 : 0;
      sto_flag = inward_asn_item_details[0].total_grn_post_qty >0? 0 :1;
    }else{
      sto_flag = 0;
    }
    if (asn_details_length == inward_asn_item_details_length)
      manual_inward_flag = inward_asn_item_details[0].total_pending_qty == 0 ? 0 : 1;

  }
  else {
    //console.log("only po",sto_flag);
    let get_po_details = await purchaseOrder.aggregate([
      {
        $match: {
          po_number: po_number
        }
    ...
    //console.log("get_po_details",get_po_details);
    let temp_item_array = [];
    get_po_details.forEach(po_element => {
      temp_item_array.push(po_element.item_no);
    })
    //console.log("temp_item_array", temp_item_array);

    let inward_po_item_details = await inwardProcess.aggregate([
      {
        $match: {
          item_no: { $in: temp_item_array },
          po_no: po_number
        }
      },
      {
    ...
    let po_details_length = get_po_details.length;
    let inward_po_item_details_length = inward_po_item_details.length;

    if (inward_po_item_details_length){
      grn_status_flag = inward_po_item_details[0].total_grn_post_qty != inward_po_item_details[0].total_net_qty ? 1 : 0
      sto_flag = inward_po_item_details[0].total_grn_post_qty >0 ? 0 :1;
    }else{
      sto_flag = 0;
    }
    if (po_details_length == inward_po_item_details_length)
      manual_inward_flag = inward_po_item_details[0].total_pending_qty == 0 ? 0 : 1;

  }
  return {
    manual_inward: manual_inward_flag,
    grn_status: grn_status_flag,
    stin_flag: sto_flag
  }
}

async function get_complete_po_asn_list(filter) {
  //console.log("filter",filter);
  const po_asn_details = await get_po_asn_details(filter);
  let complete_po_asn_response_data = [];
  if (po_asn_details.length) {
    let status_data = po_asn_details.map(async element => {
      //console.log("element",element);
      // console.log("get_grn_manual_inward_status_details",get_grn_manual_inward_status_details(element),element);
      let status = await get_grn_manual_inward_status_details(element);
      //console.log("status", status);
      complete_po_asn_response_data.push({
        po_number: element.po_number,
        asn_no: element.inbound_delivery_number ? element.inbound_delivery_number : "",
        manual_inward: status.manual_inward,
        grn_status: status.grn_status,
        stin_flag: status.stin_flag,
      })
    });

    await Promise.all(status_data)
    //console.log("get_grn_manual_inward_status",get_grn_manual_inward_status);
    //complete_po_asn_response_data.push(get_grn_manual_inward_status);
  }

  //console.log("complete_po_asn_response_data",complete_po_asn_response_data);
  return complete_po_asn_response_data;
}

//added to list po which is not inwarded
exports.po_asn_list_v2 = async (req, res) => {
  console.log("po_asn_list_v2");
  const { company_code, plant_id, delivery_date, document_date, mode, manual_inward_status } =
    req.query;
  try {
    if (!(company_code && plant_id && delivery_date && mode))
      if (!(company_code && plant_id && document_date && mode))
    ...
    //console.log("manual_inward_status",manual_inward_status,typeof manual_inward_status)
    if (manual_inward_status == "true") {
      response_data = await get_complete_po_asn_list(complete_po_list_filter);
    }
    else if (manual_inward_status == "false") {
      response_data = await get_inwarded_po_asn_list(filter);
    }
...
//added to list po which is not inwarded
exports.po_asn_list_v3 = async (req, res) => {
  console.log("po_asn_list_v3");
  const { company_code, plant_id, delivery_date, document_date, mode } =
    req.query;
  try {
    if (!(company_code && plant_id && delivery_date && mode))
    ...
    console.log("manual_inward_status",manual_inward_status);
    let filter = {};
    if (delivery_date)
      filter = {
        company_code: company_code,
