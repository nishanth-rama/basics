        ...
        if (len > 0) {
          ...
          if (
            ...
            ptlPickingQtyArr.push({
              ...
              sales_document_type:ptlPickingQty[i].sales_document_type,
  ...
  for (let i = 0; i < finalArr.length; i++) {
    if (status == "pending" && finalArr[i].ordered_qty > finalArr[i].picked_qty)
      final_arr.push({
        ...
        sales_document_type:finalArr[i].sales_document_type,
...
// Get SO TYPE
exports.get_so_type = async (req, res) => {
  const { company_code, plant_id, delivery_date,route_id } = req.query;
  try {
    if (!(company_code && plant_id && delivery_date))
      return res.status(400).send({
    ...
    const sales_document_type = await soAllocationColl.distinct("sales_document_type",filter);
    return res.status(200).send({
      status_code: 200,
      message: `SKU based SO type list is available`,
      data: sales_document_type,
    });
  } catch (err) {
    console.log(err);
    return res.status(500).send({
      status_code: 500,
      message: "Some error occurred while extracting SO type",
    });
  }
};

exports.get_STOPO_picking_qty_listV2 = async (req, res) => {
  console.log("calling get stopo details..");

  const { po_number, delivery_date, plant_id, company_code, } =
    req.query;

  try {
    if (!( delivery_date && plant_id && company_code && po_number))
      return res
        .status(400)
     ...
     let filter = {
        ...
        delivery_date: delivery_date,
        po_document_type: { $in: ["ZWST"] },
      };
   

    if (po_number != "All") filter.po_number = po_number;
    ...
    // If material_no is specified and not "All", add a $unwind stage to flatten the 'item' array
    // if (material_no && material_no !== "All") {
    //   pipeline.push({
    //     $unwind: "$item",
    //   });
    //   // Add another $match stage to filter the specific material_no
    //   pipeline.push({
    //     $match: {
    ...
    const data = await stopo.aggregate(pipeline);

    const stopo_details = data[0].item.map((ele) => ({
      selected_date: data[0].delivery_date,
      sales_order_no: data[0].po_number,
      sales_document_type: data[0].po_document_type,
      material_no: ele.material_no,
      material_name:ele.material_description,
      ordered_qty: ele.quantity,
      ordered_unit: ele.quantity,
      require_qty:ele.quantity,
      require_unit: ele.quantity,
      picked_qty: "0.00",
      picked_unit: "0.00",
      uom: ele.uom,
      status: "dispatch",
    ...
    if (stopo_details.length == 0) mssge = "STOPO data is not available!";
...
// stopo po list
exports.get_STOPO_based_po_list = async (req, res) => {
  const { company_code, plant_id, delivery_date } = req.query;
  try {
    if (!(company_code && plant_id && delivery_date))
      return res.status(400).send({
    ...
    const po_list = await stopo.find(filter, {
      _id: 0,
      po_number: 1,
    });
    ...
    if(po_list.length ==0){
      return res.status(200).send({
        status_code: 200,
        message: `stopo based po list is not available`,
        data: po_list,
      });
    }
    return res.status(200).send({
      status_code: 200,
      message: `stopo based po list is available`,
      data: po_list,
    });
  } catch (err) {
    console.log(err);
