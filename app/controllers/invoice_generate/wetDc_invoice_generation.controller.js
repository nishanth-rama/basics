...
async function sync_invoice_master(invoice_no) {
  let sap_url = `${new_sap_url}/masterToDepotSpecific_invoice/${invoice_no}`;
...
exports.download_invoice_by_number_v5 = async (req, res) => {
  ...
  try {
    ...
    let options = {
      margin: { top: "9px", right: "19px", bottom: "9px", left: "19px" },
      printBackground: false,
      format: "A4",
    };

    let invoice_array = invoice_no.split(",")


    // console.log("invoice_array",invoice_array);

    let result = []

    let result_arr = invoice_array.map(async(item,idx)=>{

      let file = { url: `https://uat-rapid.censanext.com/invoice/detail/${item}/android` };

      let pdf_buffer = await html_to_pdf.generatePdf(file, options)


      if (Buffer.byteLength(pdf_buffer) > 0) {
        result.push(pdf_buffer)
           fs.writeFileSync(`${invoice_no}.pdf`, pdf_buffer);
      }

    })


    await Promise.all(result_arr)
...
async function mergePDFs(inputPDFs, outputPDF) {
  ...
  const { PDFDocument } = require('pdf-lib');
    ...
    for (const pdfPath of inputPDFs) {
      ...
      const rmv_file = await fs.unlink(pdfPath, function (res) {
        console.log("File removed!");
      });
    ...
    return await fs.writeFile(outputPDF, mergedPdfBytes);
...
exports.download_invoice_by_number_v6 = async (req, res) => {
  // Create a browser instance
  // Create a browser instance
  ...
  try {
    ...
    let result_arr = invoice_array.map(async(item,idx)=>{
      ...
      // https://uat-api-rapid.waycool.in/download_invoice/"+invoice_no+".pdf


      if (Buffer.byteLength(pdf_buffer) > 0) {
           fs.writeFileSync(`${item}.pdf`, pdf_buffer);
           //https://uat-api-rapid.waycool.in/download_invoice/"+invoice_no+".pdf
           //https://uat-api-rapid.censanext.com/download_invoice/0902594985.pdf
           out_put_pdf = out_put_pdf + item%100
          //  console.log(path.join(__dirname,`../../../${item}.pdf`))
           const filePath = path.join(__dirname,`../../../${item}.pdf`);
    ...
    if(result.length){
      ...
      let inputPDFs = result
      out_put_pdf = out_put_pdf + ".pdf"
      try {
        await mergePDFs(inputPDFs, out_put_pdf)
        return res.send({ status_code: 200, message:"invoice pdf created successfully!",file_path: out_put_pdf })
        
      }
      catch(err){
        // console.error('Error in mergePDFs:', err.message);
        return res.status(500).send({ status_code: 500, message: err.message });
      }
    }
    else {
      return res.status(400).send({ status_code: 400, message: "Invoice pdf creation failed!" })
