...
exports.getModuleAccess = async (req,res) => {
  ...
  try {
    const getModuleAccess = await moduleNameTable.find({},
      { _id: 1, module_name: 1 }
    );
    let mssge = "Module names are available";
    if (getModuleAccess.length == 0) mssge = "Module names are not available!";

    return res.send({ message: mssge, data: getModuleAccess });
  } catch (err) {
    console.log(err);
    return res.status(500).send({
      message: "Some error occurred while fetching user module names!",
